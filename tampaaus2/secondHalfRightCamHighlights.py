...
csv_file = pandas.read_csv("C:\\Users\\DoogleSports\\pythonScripts\\tampaaus2\\top50SecondHalf.csv", delimiter=",", skipinitialspace=True)
input_file_path_stem = "D:\\TampaAus02\\RightCam\\"
output_file_path_stem = "D:\\TampaAus02\\Highlights\\"
input_file_path = input_file_path_stem + "SecondHalf.MP4"
...
def negative_shift_timestamp(input_time, delta_timestamp):
	input_time_seconds = convert_to_seconds(input_time)
	input_time_seconds -= convert_to_seconds(delta_timestamp)
	return convert_to_string_timestamp(input_time_seconds)
...
for index, row in csv_file.iterrows():
	...
	start = negative_shift_timestamp(row['start'], "01:11:25")
	#start = row['start']
	end = negative_shift_timestamp(row['end'], "01:11:25")
	#end = row['end']
	name = row['name']
	os.system("ffmpeg -i {} -vcodec copy -ss {} -to {} {}{}.{}.rc.{}.MP4".format(input_file_path, start, end, output_file_path_stem, iterator_tag, string_iterator, name))
	iterator += 1